  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled
          ? "bg-slate-950/95 backdrop-blur-lg border-b border-slate-800 shadow-lg shadow-cyan-500/10"
          : "bg-transparent"
      }`}
    >
        ...
        <div className="flex items-center justify-between h-16 lg:h-20">
          ...
          <div className="flex items-center space-x-3 group cursor-pointer">
            ...
            <div className="hidden sm:block">
              ...
              <div className="text-xs text-gray-400 font-mono">portfolio.exe</div>
            </div>
          </div>
            ...
            {navItems.map((item, index) => (
              ...
              >
                <span className="relative z-10">
                  <span className="text-cyan-400 opacity-70">0{index + 1}.</span> {item.label}
                </span>
                {activeSection === item.name && (
                  <div className="absolute inset-0 bg-cyan-500/10 rounded-lg border border-cyan-500/30"></div>
                )}
                <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r from-cyan-400 to-blue-500 scale-x-0 group-hover:scale-x-100 transition-transform duration-300"></div>
              </a>
            ))}
          </div>
          ...
          <button
            ...
            className="lg:hidden text-cyan-400 hover:text-cyan-300 transition-colors p-2"
          >
      ...
      >
        <div className="bg-slate-950/98 backdrop-blur-xl border-t border-slate-800">
          <div className="px-6 py-6 space-y-4">
            {navItems.map((item, index) => (
              <a
                ...
                className={`block font-mono transition-all duration-300 ${
                  activeSection === item.name
                    ? "text-cyan-400 pl-4 border-l-2 border-cyan-400"
                    : "text-gray-400 hover:text-cyan-300 hover:pl-4 border-l-2 border-transparent"
                }`}
              >
                <span className="text-cyan-400 opacity-70">0{index + 1}.</span> {item.label}
              </a>
            ...
            <div className="pt-4 border-t border-slate-800 flex items-center space-x-6">
              ...
              <button className="relative group flex-1">
                <div className="absolute inset-0 bg-gradient-to-r from-cyan-500 to-blue-600 rounded-lg blur-sm"></div>
